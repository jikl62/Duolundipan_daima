# 1 "../User/Bsp/Referee.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 388 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../User/Bsp/Referee.c" 2
...
void Referee_System_Frame_Update(uint8_t *Buff, uint16_t Size)
{
    ...
    for (i = 0; i < Size - 5U; )
    {
        if (Buff[i] == 0xA5)
        {
            ...
            if (Verify_CRC8_Check_Sum(&Buff[i], 5U) == 1)
            {
                ...
                if (i + data_length <= Size)
                {
                    ...
                    if (Verify_CRC16_Check_Sum(&Buff[i], data_length) == 1)
                    {

                        cmd_id = (uint16_t)(Buff[i + 5U + 1] << 8
                                          | Buff[i + 5U]);


                        data_ptr = &Buff[i + 5U + 2U];
