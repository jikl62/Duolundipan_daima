# 1 "../User/Bsp/DM_Motor.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 388 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../User/Bsp/DM_Motor.c" 2
...
# 6 "../User/Bsp/inc\\DM_Motor.h" 2
# 1 "../User/Start/Inc\\MY_define.h" 1
# 7 "../User/Bsp/inc\\DM_Motor.h" 2
...
# 2 "../User/Bsp/DM_Motor.c" 2
int16_t angleError = 0;
# 14 "../User/Bsp/DM_Motor.c"
void dm4310_fbdata(DM_MOTOR_DATA_Typdef *motor, uint8_t *rx_data)
{
    ...
    motor->pos = uint_to_float(motor->p_int, -12.5f, 12.5f, 16);
    motor->vel = uint_to_float(motor->v_int, -30.0f, 30.0f, 12);
    motor->tor = uint_to_float(motor->t_int, -10.0f, 10.0f, 12);
    motor->Tmos = (float)(rx_data[6]);
    motor->Tcoil = (float)(rx_data[7]);
    motor->ONLINE_JUDGE_TIME = 15;
...
void dm_RXdata(DM_MOTOR_Typdef *motor, uint8_t *rx_data)
{
 ...
 motor->DATA .Tmos = (float)(rx_data[7]);
  motor->DATA .reality = (int32_t)(( motor->DATA .round * 8192)+(float)(motor->DATA .Angle_now ));
...
# 110 "../User/Bsp/DM_Motor.c"
void mit_ctrl(hcan_t* hcan, uint16_t motor_id, float pos, float vel,float kp, float kd, float torq)
{
  uint8_t data[8];
  uint16_t pos_tmp,vel_tmp,kp_tmp,kd_tmp,tor_tmp;
  uint16_t id = motor_id + 0x000;

  pos_tmp = float_to_uint(pos, -12.5f, 12.5f, 16);
  vel_tmp = float_to_uint(vel, -30.0f, 30.0f, 12);
