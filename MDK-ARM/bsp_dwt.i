# 1 "../User/Bsp/bsp_dwt.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 388 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../User/Bsp/bsp_dwt.c" 2
# 36 "../User/Bsp/bsp_dwt.c"
...
void DWT_Init(uint32_t CPU_Freq_mHz)
{
    ...
    ((DWT_Type *) (0xE0001000UL) )->CYCCNT = (uint32_t)0u;
...
float DWT_GetDeltaT(uint32_t *cnt_last)
{
    volatile uint32_t cnt_now = ((DWT_Type *) (0xE0001000UL) )->CYCCNT;
...
void DWT_SysTimeUpdate(void)
{
    ...
    CYCCNT64 = (uint64_t)CYCCNT_RountCount * (uint64_t)4294967295u + (uint64_t)cnt_now;
...
void DWT_Delay(float Delay)
{
    uint32_t tickstart = ((DWT_Type *) (0xE0001000UL) )->CYCCNT;
    float wait = Delay;

    while ((((DWT_Type *) (0xE0001000UL) )->CYCCNT - tickstart) < wait * (float)CPU_FREQ_Hz)
    {
    }
}
...
void DWT_Delay_us(uint32_t us)
{
    uint32_t start_tick = ((DWT_Type *) (0xE0001000UL) )->CYCCNT;
    uint32_t delay_ticks = us * CPU_FREQ_Hz_us;

    while ((((DWT_Type *) (0xE0001000UL) )->CYCCNT - start_tick) < delay_ticks)
    {
