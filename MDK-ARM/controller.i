# 1 "../User/Algorithm/controller.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 388 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../User/Algorithm/controller.c" 2
# 64 "../User/Algorithm/controller.c"
...
# 65 "../User/Algorithm/controller.c" 2
...
# 249 "../User/Algorithm/controller.c"
float PID_Calculate(PID_t *pid, float measure, float ref)
{
...
static void f_Changing_Integration_Rate(PID_t *pid)
{
    if (pid->Err * pid->Iout > 0)
    {
        ...
        if (((pid->Err > 0) ? pid->Err : -pid->Err) <= pid->CoefB)
            return;
        if (((pid->Err > 0) ? pid->Err : -pid->Err) <= (pid->CoefA + pid->CoefB))
            pid->ITerm *= (pid->CoefA - ((pid->Err > 0) ? pid->Err : -pid->Err) + pid->CoefB) / pid->CoefA;
        else
            pid->ITerm = 0;
    }
}
...
static void f_Integral_Limit(PID_t *pid)
{
    ...
    if (((temp_Output > 0) ? temp_Output : -temp_Output) > pid->MaxOut)
    {
        if (pid->Err * pid->Iout > 0)
        {
...
# 538 "../User/Algorithm/controller.c"
void Feedforward_Init(
    Feedforward_t *ffc,
    float max_out,
    float *c,
...
# 588 "../User/Algorithm/controller.c"
float Feedforward_Calculate(Feedforward_t *ffc, float ref)
{
...
# 607 "../User/Algorithm/controller.c"
...
float LDOB_Calculate(LDOB_t *ldob, float measure, float u)
{
    ...
    if (((ldob->Disturbance > 0) ? ldob->Disturbance : -ldob->Disturbance) > ldob->DeadBand * ldob->Max_Disturbance)
