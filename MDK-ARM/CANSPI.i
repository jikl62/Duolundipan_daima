...
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../User/Bsp/CANSPI.c" 2
# 23 "../User/Bsp/CANSPI.c"
# 1 "../User/Bsp/inc\\CANSPI.h" 1
# 26 "../User/Bsp/inc\\CANSPI.h"
# 1 "../Core/Inc\\main.h" 1
# 30 "../Core/Inc\\main.h"
...
# 24 "../User/Bsp/CANSPI.c" 2
# 1 "../User/Bsp/inc\\MCP2515.h" 1
# 27 "../User/Bsp/inc\\MCP2515.h"
# 1 "D:\\kevil\\ARM\\ARMCLANG\\Bin\\..\\include\\stdbool.h" 1 3
# 28 "../User/Bsp/inc\\MCP2515.h" 2
...
void CANSPI_Sleep(SPI_HandleTypeDef *hspi)
{
  ...
  MCP2515_BitModify(hspi, 0x2C, 0x40, 0x00);


  MCP2515_BitModify(hspi, 0x2B, 0x40, 0x40);
...
uint8_t CANSPI_Initialize(SPI_HandleTypeDef *hspi)
{
  ...
  MCP2515_WriteByteSequence(hspi, 0x20, 0x23, &(RXM0reg.RXM0SIDH));
  MCP2515_WriteByteSequence(hspi, 0x24, 0x27, &(RXM1reg.RXM1SIDH));
  MCP2515_WriteByteSequence(hspi, 0x00, 0x03, &(RXF0reg.RXF0SIDH));
  MCP2515_WriteByteSequence(hspi, 0x04, 0x07, &(RXF1reg.RXF1SIDH));
  MCP2515_WriteByteSequence(hspi, 0x08, 0x0B, &(RXF2reg.RXF2SIDH));
  MCP2515_WriteByteSequence(hspi, 0x10, 0x13, &(RXF3reg.RXF3SIDH));
  MCP2515_WriteByteSequence(hspi, 0x14, 0x17, &(RXF4reg.RXF4SIDH));
  MCP2515_WriteByteSequence(hspi, 0x18, 0x1B, &(RXF5reg.RXF5SIDH));


  MCP2515_WriteByte(hspi, 0x60, 0x04);
  MCP2515_WriteByte(hspi, 0x70, 0x01);
# 143 "../User/Bsp/CANSPI.c"
  MCP2515_WriteByte(hspi, 0x2A, 0x00);


  MCP2515_WriteByte(hspi, 0x29, 0x91);


  MCP2515_WriteByte(hspi, 0x28, 0x01);

  MCP2515_WriteByte(hspi, 0x2C,0x00);
  MCP2515_WriteByte(hspi, 0x2B,0x01);
...
uint8_t CANSPI_Transmit(SPI_HandleTypeDef *hspi, uCAN_MSG *tempCanMsg)
{
  ...
  if (ctrlStatus.TXB0REQ != 1)
  {
    ...
    MCP2515_LoadTxSequence(hspi, 0x40, &(idReg.tempSIDH), tempCanMsg->frame.dlc, &(tempCanMsg->frame.data0));


    MCP2515_RequestToSend(hspi, 0x81);
  ...
  else if (ctrlStatus.TXB1REQ != 1)
  {
    ...
    MCP2515_LoadTxSequence(hspi, 0x42, &(idReg.tempSIDH), tempCanMsg->frame.dlc, &(tempCanMsg->frame.data0));
    MCP2515_RequestToSend(hspi, 0x82);
  ...
  else if (ctrlStatus.TXB2REQ != 1)
  {
    ...
    MCP2515_LoadTxSequence(hspi, 0x44, &(idReg.tempSIDH), tempCanMsg->frame.dlc, &(tempCanMsg->frame.data0));
    MCP2515_RequestToSend(hspi, 0x84);
...
uint8_t CANSPI_Receive(SPI_HandleTypeDef *hspi, uCAN_MSG *tempCanMsg)
{
  ...
  if (rxStatus.rxBuffer != 0)
  {
    ...
    if ((rxStatus.rxBuffer == 0x01)|(rxStatus.rxBuffer == 0x03))
    {
      MCP2515_ReadRxSequence(hspi, 0x90, rxReg.rx_reg_array, sizeof(rxReg.rx_reg_array));
    }
    else if (rxStatus.rxBuffer == 0x02)
    {
      MCP2515_ReadRxSequence(hspi, 0x94, rxReg.rx_reg_array, sizeof(rxReg.rx_reg_array));
    }


    if (rxStatus.msgType == 2)
    {
...
void WHW_MCP2515_IRQHandler(SPI_HandleTypeDef *hspi)
{
 uint8_t temp = MCP2515_ReadByte(hspi, 0x2C);
 if(temp)
 {
  if(temp & 0x01)
   MCP2515_WriteByte(hspi, 0x2C,0);
  else
   temp--;
 }
 else
...
uint8_t CANSPI_isBussOff(SPI_HandleTypeDef *hspi)
{
  ...
  errorStatus.error_flag_reg = MCP2515_ReadByte(hspi, 0x2D);
