# 1 "../User/Start/WHW_IRQN.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 388 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../User/Start/WHW_IRQN.c" 2
...
void StartMoveTask(void const * argument)
{
    ...
    RUI_V_CONTAL.CG.YAW_INIT_ANGLE = 6680;
...
{
    TickType_t currentTimeDefiant;
    currentTimeDefiant = xTaskGetTickCount();


    RUI_V_CONTAL.SHOOT.Shoot_Speed = 2500;
    RUI_V_CONTAL.SHOOT.Single_Angle = -36864;
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef* hcan)
{
 CAN_RxHeaderTypeDef can_rx;
    ...
    HAL_CAN_GetRxMessage(hcan , (0x00000000U) , &can_rx , rx_data);
...
void BSP_UART_IRQHandler(UART_HandleTypeDef *huart)
{
    ...
    {
        if (RESET != (((&huart3)->Instance->SR & (((uint32_t)(0x1UL << (4U))))) == (((uint32_t)(0x1UL << (4U))))))
        {
   uint16_t temp = 0;
            do{ volatile uint32_t tmpreg = 0x00U; tmpreg = (&huart3)->Instance->SR; tmpreg = (&huart3)->Instance->DR; (void)tmpreg; } while(0U);
   temp = huart3.Instance -> SR;
   temp = huart3.Instance -> DR;

            HAL_UART_DMAStop(&huart3);

            if((37) - ((&hdma_usart3_rx)->Instance->NDTR) == 18)
...
# 360 "../User/Start/WHW_IRQN.c"
}
# 376 "../User/Start/WHW_IRQN.c"
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size){
 ...
 if (huart->Instance == ((USART_TypeDef *) ((0x40000000UL + 0x00010000UL) + 0x1400UL))){
        uint8_t *next_buf = (pData == Referee_Rx_Buf[0]) ? Referee_Rx_Buf[1] : Referee_Rx_Buf[0];
        HAL_UARTEx_ReceiveToIdle_DMA(huart, next_buf, 256);
