# 1 "../User/Algorithm/heat_control.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 388 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../User/Algorithm/heat_control.c" 2
...
# 6 "../User/Algorithm/inc\\heat_control.h" 2
...
# 2 "../User/Algorithm/heat_control.c" 2
...
# 56 "../User/Algorithm/heat_control.c"
  ...
  float shots_fired = (float)delta_shots;
    ...
    g_heat_watcher.local_heat += (shots_fired * 10.0f);
  ...
  g_heat_watcher.shots_allowed=g_heat_watcher.remain_heat/ 10.0f;
...
_Bool Update_Shoot_Det(float speed1, float speed2, ShootDet_t *det) {
    ...
    det->last_val = val;
    if (val > det->base) {
        det->base = (0.360f * val) + (1.0f - 0.360f) * det->base;
    } else {
        det->base = (0.059f * val) + (1.0f - 0.059f) * det->base;
    }
    ...
    if (!det->armed) {
        ...
        _Bool condition_relative = (drop < det->max_drop_in_round * (1.0f - 0.15f));
        _Bool condition_absolute = (drop < 100.0f);
